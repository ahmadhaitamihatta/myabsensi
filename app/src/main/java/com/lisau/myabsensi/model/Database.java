...
public class Database extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + TABLE_NAME + " (" + ID + " TEXT PRIMARY KEY," + NIM + " TEXT," + NAMA + " TEXT," + TANGGAL + " TEXT," + JAM + " TEXT," + STATUS + " TEXT," + TELAT + " TEXT " + ")");
    }
    ...
    public boolean TambahData(String id, String nim, String nama, String tanggal, String jam, String status, String telat) {

        //cek data nik jika ada di database
        if (CekDataAbsen(nim, tanggal)) {
    ...
    public Cursor TampilDataAbsen() {
        SQLiteDatabase db = this.getWritableDatabase();
        String[] columns = {ID, NIM, NAMA, TANGGAL, JAM, STATUS, TELAT};
        return db.query(TABLE_NAME, columns, null, null, null, null, null);
    }

    //Prosedure Cek Data
    public Boolean CekDataAbsen(String nim, String tanggal) {
        SQLiteDatabase db = this.getWritableDatabase();

        Cursor cursor = db.rawQuery("SELECT nim FROM absensi WHERE nim = '" + nim + "' AND tanggal = '" + tanggal + "'", null);
        if (cursor != null) {
